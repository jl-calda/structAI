...
import { Field2, Legend, RebarBlock } from '@/components/beams/RebarBlock'
import { RebarRow, type BentMode } from '@/components/beams/RebarRow'
...
export function BeamReinforcementCard({
  b,
  h,
  span,
  cover,
  ...
  // Sync bent[] arrays with bar counts
  useEffect(() => {
    setT1Bent(prev => {
      const next = [...prev]
  ...
  // Compute effective bar layout AT the active section (after bent-up redistribution)
  ...
  const sectionCtx = [
    { k: 'start' as const, n: '§1', t: 'Start · @ support i', m: `M⁻ ${forces.mNegPeak.toFixed(1)}`, v: `V ${forces.vPeak.toFixed(1)}` },
    { k: 'mid' as const, n: '§2', t: 'Mid · @ midspan', m: `M⁺ ${forces.mPosPeak.toFixed(1)}`, v: `V ${(forces.vPeak * 0.25).toFixed(1)}` },
    { k: 'end' as const, n: '§3', t: 'End · @ support j', m: `M⁻ ${(forces.mNegPeak * 0.92).toFixed(1)}`, v: `V ${(forces.vPeak * 0.94).toFixed(1)}` },
  ]
  ...
  return (
    <div className="card" data-step="2-reinforcement">
      <div className="card-h">
        ...
        <div className="right">
          <div className="result-bar" style={{ margin: 0, padding: '4px 10px' }}>
            <span className="label">As</span>
            <span className={asPass ? '' : 'fail'} style={{ color: asPass ? 'var(--color-pass)' : 'var(--color-fail)' }}>
              {Math.round(asT)} mm²
            </span>
            <span style={{ color: 'var(--color-ink-3)' }}>≥</span>
            <span>{asReq}</span>
            <span style={{ color: asPass ? 'var(--color-pass)' : 'var(--color-fail)' }}>{asPass ? '✓' : '✗'}</span>
          </div>
        </div>
      </div>
      ...
      <div className="card-b" style={{ padding: 0, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
        ...
        <div style={{ padding: 10, borderRight: '1px solid var(--color-line-2)', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <RebarBlock title="Bottom As · continuous" color="#B06008" hint="bars run full length — ⇈ to bend up">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto', gap: 6, alignItems: 'center', padding: '4px 0 0 28px' }}>
              ...
              <span className="mono" style={{ fontSize: 9.5, color: 'var(--color-ink-4)' }}>
                = {Math.round(span * bendFrac)} mm
              </span>
            </div>
          ...
          <RebarBlock title="Stirrups" color="#1755A0">
            <div style={{ display: 'grid', gridTemplateColumns: '24px 1fr 1fr', gap: 6, alignItems: 'center', marginBottom: 4 }}>
              ...
              <select className="select" value={stirLegs} onChange={e => setStirLegs(Number.parseInt(e.target.value, 10) as 2 | 3 | 4 | 6)} style={{ height: 22, fontSize: 11 }}>
                {[2, 3, 4, 6].map(d => <option key={d} value={d}>{d}-leg</option>)}
              </select>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '24px 1fr 1fr', gap: 6, alignItems: 'center' }}>
              <span className="mono" style={{ fontSize: 10.5, color: 'var(--color-ink-3)' }}>s</span>
              <Spinner value={stirSpacingEnd} onChange={setStirSpacingEnd} step={25} min={50} />
              <Spinner value={stirSpacingMid} onChange={setStirSpacingMid} step={25} min={50} />
            </div>
            <div style={{ fontSize: 9.5, color: 'var(--color-ink-4)', marginTop: 4, paddingLeft: 28 }}>
              <span style={{ marginRight: 12 }}>end zone @ {stirSpacingEnd} mm</span>
              <span>mid zone @ {stirSpacingMid} mm</span>
            </div>
          </RebarBlock>
        </div>
        ...
        <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div className="sub-label">Capacity at this section</div>
          <table className="t">
            <tbody>
              <tr><td>As provided</td><td className="num">{Math.round(asT)}</td><td>mm²</td></tr>
              <tr><td>As required</td><td className="num">{asReq}</td><td>mm²</td></tr>
              <tr className={asPass ? 'govern' : 'fail'}>
                <td>Ratio</td>
                <td className="num">{(asT / asReq).toFixed(2)}</td>
                <td>{asPass ? '✓' : '✗'}</td>
              </tr>
            </tbody>
          </table>
